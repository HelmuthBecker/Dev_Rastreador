...
char gruposEquipes[] = {'A','B'};                             // Grupos de ID's das equipes que estão competindo na prova - Cada grupo pode conter no máximo 10 equipes
byte indice;                                                  // Quantidade de equipes
char mensagem[491];                      //mensagem contendo os dados recebidos dos módulos transmissores
...
SoftwareSerial mySerial(12,6);           //Software serial do módulo LORA (7-Rx - 3-Tx)
EBYTE emissor(&mySerial, 3, 5, 2);       //Objeto Ebyte
...
void Receber(){                                                //Função que recebe os dados dos módulos
    for (unsigned int a = 0; a < 490; a++ ){                   //Limpa a variavel mensagem, antes do uso
     ...
     bytesRecebidos = mySerial.readBytesUntil('@', mensagem, 490);    //Recebe dados no buffer até encontrar o caracter * que indica o fim da mensagem
     }
  }   //Fim Receber

void solicitar(){
  unsigned long pausa;                    
  indice = (sizeof(gruposEquipes));                     //Define quantas equipes estão competindo de acordo com o tamanho da var equipes
 ...
 digitalWrite(LED9, LOW);
    ...
    Serial.println(gruposEquipes[a]);    
    mySerial.print(gruposEquipes[a]);
